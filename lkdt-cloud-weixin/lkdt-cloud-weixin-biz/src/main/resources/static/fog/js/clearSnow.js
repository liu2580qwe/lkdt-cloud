...
/***雾情详细**/
function alarmDetails(hwName,alarmDate){
    ...
    layer.open({
        ...
        area : [ '80%', '80%' ],
        zIndex:99999999,
        content : "/report/alarmReport/alarmRoadReportDetails?hwName="+hwName+"&alarmDate="+alarmDate
    });
}
...
// 		let colors = "['#1790cf','#1bb2d8','#99d2dd','#88b0bb','#1c7099','#038cc4','#75abd0','#afd6dd']";
    ...
    myChartYearReport.on('click', function (params) {
        ...
        // }
        let now = new Date()
        alarmDetails(vHwName, formatDate(now));
    });
 ...
 * 事件检测点击事件
 */
function contentUl_event_click(ele){
    layer.open({
        type : 2,
        title : "事件检测",
        maxmin : true,
...
function 测试按钮(epId,name,equLocation){
    epId = '88';
    layer.open({
        type : 2,
        title : "ID：" + epId + " >> 桩号：" + name + " >> 位置：" + equLocation,
