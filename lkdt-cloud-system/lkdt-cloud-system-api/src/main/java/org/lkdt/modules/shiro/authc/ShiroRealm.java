package org.lkdt.modules.shiro.authc;
...
//	private SysBaseRemoteApi sysBaseRemoteApi;
    ...
    public boolean jwtTokenRefresh(String token, String userName, String passWord) {
        String cacheToken = String.valueOf(redisUtil.get(CacheConstant.SYS_USERS_CACHE_JWT + ":" + token));
        if (oConvertUtils.isNotEmpty(cacheToken)) {
            // 校验token有效性
            if (!JwtUtil.verify(cacheToken, userName, passWord)) {
//				String newAuthorization = JwtUtil.sign(userName, passWord);
                // 设置超时时间
//				redisUtil.set(CacheConstant.SYS_USERS_CACHE_JWT + ":" + token, newAuthorization);
                //根据过期时间判断是否app请求
                if (redisUtil.getExpire(CacheConstant.SYS_USERS_CACHE_JWT + ":" + token) > JwtUtil.EXPIRE_TIME * 2 / 1000) {
