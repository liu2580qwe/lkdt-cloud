...
import org.lkdt.common.util.ImportExcelUtil;
import org.lkdt.common.util.oConvertUtils;
import org.lkdt.modules.system.entity.SysDepart;
import org.lkdt.modules.system.model.DepartIdModel;
import org.lkdt.modules.system.model.SysDepartTreeModel;
...
//			}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryDepartListByUid", method = RequestMethod.GET)
	public Result<List<SysDepartModel>> queryDepartListByUid(@RequestParam(name = "userId", required = true) String userId) {
		Result<List<SysDepartModel>> result = new Result<>();
		try {

			List<SysDepart> sysDepartList = sysDepartService.queryUserDeparts(userId);
			...
			result.setResult(sysDepartModelList);
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(),e);
		}
...
//		}
	 ...
	 * @return
	 */
	@GetMapping("listAllDepart")
	public List<SysDepart> listAllDepart() {
		LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
		query.orderByAsc(SysDepart::getOrgCode);
		List<SysDepart> ls = this.sysDepartService.list(query);
		return ls;
