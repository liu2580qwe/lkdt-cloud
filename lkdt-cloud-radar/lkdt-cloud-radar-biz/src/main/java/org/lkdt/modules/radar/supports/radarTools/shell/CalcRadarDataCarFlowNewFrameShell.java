 ...
 * 计算雷达来向和去向车流量
 */
public class CalcRadarDataCarFlowNewFrameShell {
    ...
    public static void main(String[] args) {
        ...
        DefaultCategoryDataset dataSetCarFlowLaiStart = new DefaultCategoryDataset();
        DefaultCategoryDataset dataSetCarFlowLaiEnd = new DefaultCategoryDataset();
        DefaultCategoryDataset dataSetCarFlowQu = new DefaultCategoryDataset();
        DefaultCategoryDataset dataSetCarFlowQuStart = new DefaultCategoryDataset();
        DefaultCategoryDataset dataSetCarFlowQuEnd = new DefaultCategoryDataset();
...
//                                        //新算法滤噪
//                                        if(zcLdEventRadarInfoPool.getEndCoordinateY() == null){
//                                            if(zcLdEventRadarInfoPool.getBeginCoordinateY() < 101){
//
//                                            }
//                                            //每个目标第二帧
//                                            double beginY = zcLdEventRadarInfoPool.getBeginCoordinateY().doubleValue();
//                                            double currY = sY_;
//                                            double v = Math.abs(vY_);
//                                            double t = (currY - beginY)/v;
//                                            if(t > 0.09){
//                                                //不符合条件过滤掉
//                                                commonPool.remove("" + targetId_);
//                                                continue;
//                                            }
//                                        } else {
//                                            //每个目标第N帧
                                    ...
                                    if(!dataFrameList.contains(map.getKey())){
                                        ...
                                        if(diuShiData != null){
                                            ...
                                            if(validFilter(diuShiData)){
                                                //未去噪
                                                //新流量算法去噪
                                                JSONArray jsonArray = diuShiData.getJsonArray();
                                                if(jsonArray != null && jsonArray.size() > 0){
                                                    //平均速度  去噪,方法1
                                                    double avgV = 0;
                                                    for(int jI = 0; jI < jsonArray.size(); jI++){
                                                        JSONObject jsonObject = jsonArray.getJSONObject(jI);
                                                        avgV += Math.abs(jsonObject.getDouble("vY"));
                                                        //去噪,方法2
                                                        if(jI > 0){
                                                            JSONObject jsonObject_1 = jsonArray.getJSONObject(jI-1);
                                                            double yN = jsonObject.getDouble("sY");
                                                            double yN_1 = jsonObject_1.getDouble("sY");
                                                            double xN = jsonObject.getDouble("sX");
                                                            double xN_1 = jsonObject_1.getDouble("sX");

                                                        }
                                                    }
                                                    avgV /= jsonArray.size();
                                                    //新流量算法去噪
                                                    JSONObject objStart0 = jsonArray.getJSONObject(0);
                                                    JSONObject objStart1 = jsonArray.getJSONObject(1);
                                                    JSONObject objEndN_1 = jsonArray.getJSONObject(jsonArray.size() - 1);
                                                    JSONObject objEndN_2 = jsonArray.getJSONObject(jsonArray.size() - 2);
                                                    double sY0 = objStart0.getDouble("sY");
                                                    double sY1 = objStart1.getDouble("sY");
                                                    double sYN_1 = objEndN_1.getDouble("sY");
                                                    double sYN_2 = objEndN_2.getDouble("sY");
                                                    double deltY_start = Math.abs(sY1 - sY0);
                                                    double deltY_end = Math.abs(sYN_1 - sYN_2);

                                                    double sX0 = objStart0.getDouble("sX");
                                                    double sX1 = objStart1.getDouble("sX");
                                                    double sXN_1 = objEndN_1.getDouble("sX");
                                                    double sXN_2 = objEndN_2.getDouble("sX");
                                                    double deltX_start = Math.abs(sX1 - sX0);
                                                    double deltX_end = Math.abs(sXN_1 - sXN_2);
                                                    ...
                                                    if(diuShiData.getLaneEndRadar() < 4){
                                                        ...
                                                        if(diuShiData.getBeginCoordinateY().doubleValue() > 240 && diuShiData.getBeginCoordinateY().doubleValue() < 260){
                                                            if(Math.abs(sY1 - sY0)/avgV < 0.09){
                                                                if(deltY_start < 3.5 && deltX_start < 1){
                                                                    countLaiStart++;
                                                                    dataSetCarFlowLaiStart.addValue(diuShiData.getBeginCoordinateY().doubleValue(), "来向开始Y值", sdf.format(diuShiData.getBeginTime()));
                                                                }
                                                            }
                                                        }
                                                        if(diuShiData.getEndCoordinateY().doubleValue() > 90 && diuShiData.getEndCoordinateY().doubleValue() < 110){
                                                            if(Math.abs(sYN_1 - sYN_2)/avgV < 0.09){
                                                                if(deltY_end < 3.5 && deltX_end < 1){
                                                                    countLaiEnd++;
                                                                    dataSetCarFlowLaiEnd.addValue(diuShiData.getEndCoordinateY().doubleValue(), "来向结束Y值", sdf.format(diuShiData.getBeginTime()));
                                                                }
                                                            }
                                                        }
                                                        ...
                                                        if(diuShiData.getBeginCoordinateY().doubleValue() > 90 && diuShiData.getBeginCoordinateY().doubleValue() < 110){
                                                            if(Math.abs(sY1 - sY0)/avgV < 0.09){
                                                                if(deltY_start < 3.5 && deltX_start < 1){
                                                                    countQuStart++;
                                                                    dataSetCarFlowQuStart.addValue(diuShiData.getBeginCoordinateY().doubleValue(), "去向开始Y值", sdf.format(diuShiData.getBeginTime()));
                                                                }
                                                            }
                                                        }
                                                        if(diuShiData.getEndCoordinateY().doubleValue() > 240 && diuShiData.getEndCoordinateY().doubleValue() < 260) {
                                                            if(Math.abs(sYN_1 - sYN_2)/avgV < 0.09){
                                                                if(deltY_end < 3.5 && deltX_end < 1){
                                                                    countQuEnd++;
                                                                    dataSetCarFlowQuEnd.addValue(diuShiData.getEndCoordinateY().doubleValue(), "去向结束Y值", sdf.format(diuShiData.getBeginTime()));
                                                                }
                                                            }
                                                        }
...
//                                                }
            ...
            generateLineJPEG(dataSetCarFlowLaiStart, "1001车流量折线图", "来向进雷达坐标","时间" ,
                    "C:\\Users\\zhangzhenbiao\\Desktop\\来向进雷达Y坐标.jpg");
            generateLineJPEG(dataSetCarFlowLaiEnd, "1001车流量折线图", "来向出雷达坐标","时间" ,
                    "C:\\Users\\zhangzhenbiao\\Desktop\\来向出雷达Y坐标.jpg");
            generateLineJPEG(dataSetCarFlowQuStart, "1001车流量折线图", "去向进雷达坐标","时间" ,
                    "C:\\Users\\zhangzhenbiao\\Desktop\\去向进雷达Y坐标.jpg");
            generateLineJPEG(dataSetCarFlowQuEnd, "1001车流量折线图", "去向出雷达坐标","时间" ,
                    "C:\\Users\\zhangzhenbiao\\Desktop\\去向出雷达Y坐标.jpg");

        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            System.out.println("总计数:" + countLai + "【来向】，" + countQu + "【去向】");
            System.out.println("总计数:" + countLaiStart + "【来向进】，" + countLaiEnd + "【来向出】" + countQuStart + "【去向进】，" + countQuEnd + "【去向出】");
            System.out.println("车道1：" + count1);
            System.out.println("车道2：" + count2);
            System.out.println("车道3：" + count3);
            System.out.println("车道4：" + count4);
            System.out.println("车道5：" + count5);
            System.out.println("车道6：" + count6);
            System.out.println("车道7：" + count7);
            //释放资源
...
//            categoryAxis.setTickLabelFont(new Font("宋体" , Font.BOLD , 18));
            ...
            try{
                ...
                ChartUtils.writeChartAsJPEG(baos, chart, 5000,1000,null);
                baos.flush();
                baos.close();
