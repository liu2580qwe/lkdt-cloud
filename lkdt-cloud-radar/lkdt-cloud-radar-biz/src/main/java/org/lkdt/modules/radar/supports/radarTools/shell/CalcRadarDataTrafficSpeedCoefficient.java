 ...
 * 通行速度指数 按分钟
 */
public class CalcRadarDataTrafficSpeedCoefficient {

    private static Logger logger = LoggerFactory.getLogger(CalcRadarDataTrafficSpeedCoefficient.class);
        ...
        Thread thread = new Thread(() -> {
            while(true){
                try {
                    ...
                    //System.out.println("******结束任务：" + end + " 耗时：" + (end - start));
    ...
    public static void threeStatusCalc() {
        ...
        for(Map.Entry<String, RoadFlowData> $map: roadFlowDataMap.entrySet()){
                ...
                if(list != null){
                    if(num == 0){
                        ...
                        for(RadarObjDO radarObjDO: list){
                            if(radarObjDO.getsY() >= 100 && radarObjDO.getsY() <= 200){//过滤条件
                        ...
                        for(Map.Entry<Integer, RadarObjDO> map: persistFrame.entrySet()){
                            if(!targetIds.contains(map.getKey())){
                                //System.out.printf("targetIds:%s map.getKey():%s\n", targetIds.toString(), map.getKey());
            ...
            if(distance == null || distance > 200){
                int nSmall = vSmallList.size();//小车数量
                int nSmallHun = vSmallListHun.size();
                int mBig = vBigList.size();//大车数量
                ...
                System.out.printf("【道路交通流三态算法计算】pv=%s，超车道数量：小车%s大车%s，混合车道数量小车%s大车%s\n", pV, nSmall, mBig, nSmallHun, mBigHun);
            }
...
//            }
        ...
        if(roadFlowData != null){
            if(roadFlowData.timeSwitch) {
                roadFlowData.arrayQTrue.offer(radarObjDOList);
            } else {
                roadFlowData.arrayQFalse.offer(radarObjDOList);
            }
